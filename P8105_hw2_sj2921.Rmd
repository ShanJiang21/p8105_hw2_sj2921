...
```
**Mr.Trash Wheel Data Overview**: 
```{r}
str(trw_df)
```

1. There are in total **285** observations and **14** variables in the Mr.Trash Wheel dataframe.
2. Key variables included are month, weight and volume of the dumpster, and the various types of trash collected in dumpster. 
...
```
Month Variable Coversion and Precipitation summary
```{r 2.4}
...
str(Precp_df)
```
1. There are in total **12 observations** and 3 variables in the 2017 precipitation data, also 12 observations and 3 variables for 2016. After a join operation was carried, the total variables become 5 as the month is key to join, there are no NAs in the dataframe.
2. Key variables are month in each year and total precipitation in each year.
